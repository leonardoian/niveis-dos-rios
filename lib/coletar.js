...
import { calcularVelocidade, calcularEtaCota, avaliarEstimativa } from './calculo.js';
import { enviarNotificacoesAlerta } from './push.js';
import { buscarNiveisAna, ESTACOES_ANA, calcularChuvaAcumulada } from './ana.js';
...
export async function executarColeta() {
  ...
  await enviarNotificacoesAlerta([...novosAlertas, ...novosAlertasChuva]);
...
async function registrarLeiturasAna() {
  ...
  for (const l of leituras) {
    const r = await sql`
      ...
      VALUES (${l.slug}, ${l.nivel}, ${l.medidoEm}, ${l.chuvaMm})
      ON CONFLICT (slug, medido_em) DO NOTHING
      RETURNING id
    `;
    if (r.length > 0) inseridas += 1;
