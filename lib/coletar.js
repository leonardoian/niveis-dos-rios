...
//
// projecoes_gravadas = 0 é normal em 3 de cada 4 rodadas: as projeções são
// horárias e a coleta é a cada 15 min (ver registrarProjecoesNivel). É a
// SOMA na janela que diz se está saudável, não a linha isolada — por isso
// /api/saude soma por janela em vez de olhar a última rodada.
  ...
  try {
    await sql`
      INSERT INTO coletas (
        ...
        projecoes_gravadas, erro_projecoes
      ) VALUES (
        ...
        ${r.projecoesGravadas?.total ?? 0}, ${r.erroProjecoes ?? null}
      )
    `;
  } catch (erro) {
...
async function coletar() {
  ...
  //
  // Mas engolir o erro em silêncio criava outro problema: com a tabela
  // faltando em produção, a coleta seguia verde e projecoes_nivel ficava
  // vazia sem nada denunciar. Por isso a mensagem vai pro resultado da
  // rodada e daí pra coluna erro_projecoes de `coletas` — mesmo tratamento
  // que erro_feed já tinha —, e aparece em /api/saude.
  let projecoesGravadas = null;
  let projecoesAvaliadas = null;
  let erroProjecoes = null;
  try {
...
//
// Cadência: UMA emissão por hora por estação, não uma por coleta — o
// NOT EXISTS de cada INSERT só deixa passar o trio (slug, metodo,
// horizonte_h) que ainda não tem linha na hora corrente.
//
// O motivo é estatístico, não de custo: projeções emitidas a cada 15 min são
// quase idênticas entre si e seus erros são fortemente autocorrelacionados.
// Contar as quatro da hora como quatro observações independentes inflaria
// artificialmente o n da análise — um n grande obtido assim não aumenta a
// informação, só a aparência dela, e qualquer intervalo de confiança
// calculado em cima sairia estreito demais. De quebra, o volume cai de ~10,7
// mil pra ~2,7 mil linhas/dia.
//
// A trava é determinística e idempotente de propósito: depende só de
// date_trunc('hour', gerada_em), não de "quando foi a última vez" guardado
// em algum lugar. Rodar a coleta cinco vezes na mesma hora grava uma vez;
// rodar zero vezes numa hora simplesmente perde aquela hora, sem
// compensação retroativa depois (que seria backfill).
//
// Não é a trava de "uma em aberto por vez" da estimativas_cota: aqui não se
// espera a projeção anterior ser avaliada — uma projeção de 48h ficaria
// pendente dois dias e bloquearia todas as outras.
//
// gerada_em é o MESMO instante pra todas as linhas da rodada, de propósito:
// é o que permite agrupar depois "todas as projeções emitidas às 14h" e o
// que o índice único por hora usa pra não duplicar.
//
...
async function registrarProjecoesNivel() {
  ...
  // registrarLeiturasAna: são ~112 linhas na rodada que abre a hora (14
  // estações × 4 horizontes × 2 métodos) e o driver da Neon é HTTP, então
  // um INSERT por linha viraria 112 round-trips.
  const slugs = [];
  const metodos = [];
  const horizontes = [];
  const niveis = [];
  ...
  const [{ gravadas }] = await sql`
    WITH novas AS (
      ...
      -- A trava horária. Escrita com a MESMA expressão do índice único
      -- (date_trunc sobre AT TIME ZONE 'UTC') pra que o planner possa usá-lo
      -- em vez de varrer a tabela — que cresce pra sempre.
      WHERE NOT EXISTS (
        SELECT 1 FROM projecoes_nivel pn
        WHERE pn.slug = u.slug
          AND pn.metodo = u.metodo
          AND pn.horizonte_h = u.horizonte_h
          AND date_trunc('hour', pn.gerada_em AT TIME ZONE 'UTC')
            = date_trunc('hour', ${geradaEm}::timestamptz AT TIME ZONE 'UTC')
      )
      -- Sem alvo explícito: a trava real é o índice único por hora, que é
      -- por expressão. Ele fecha a corrida que o NOT EXISTS sozinho não
      -- fecha (duas rodadas simultâneas — Vercel e GitHub Actions batendo
      -- na mesma hora — ambas veriam a tabela sem a linha).
...
// ~112 de uma vez (a hora inteira vence junto), e um loop seria mais de cem
// round-trips HTTP. A escolha da leitura fica no LATERAL, que é o
// equivalente em SQL do "mais próxima do alvo" que avaliarEstimativa faz
// em JS.
//
