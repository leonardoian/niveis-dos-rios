...
import { buscarPrevisao } from './previsao.js';
...
export async function executarColeta() {
  ...
  // Independe do feed de nível ter respondido ou não — a previsão de vazão
  // é uma fonte separada, autolimitada a 1x/dia por estação.
...
// Busca a previsão de vazão de cada estação no máximo 1x/dia — a API do
// Open-Meteo só atualiza a previsão diariamente, então repetir a cada 15 min
// só gastaria chamada à toa. gerado_em::date = CURRENT_DATE é o que garante
// esse limite, independente de quantas vezes a coleta rodar no mesmo dia.
async function atualizarPrevisoes() {
  ...
  for (const e of estacoes) {
    ...
    try {
      const pontos = await buscarPrevisao(e.lat, e.lon);
      for (const p of pontos) {
        await sql`
          INSERT INTO previsoes (slug, dia, vazao_m3s, gerado_em)
          VALUES (${e.slug}, ${p.dia}, ${p.vazaoM3s}, NOW())
          ON CONFLICT (slug, dia) DO UPDATE
            SET vazao_m3s = EXCLUDED.vazao_m3s, gerado_em = NOW()
        `;
      }
      atualizadas.push(e.slug);
    } catch (erro) {
      // Falha na previsão de uma estação não pode derrubar a coleta de nível.
      console.error(`Falha ao buscar previsão de vazão de ${e.slug}:`, erro.message);
    }
  }
