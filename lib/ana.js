...
// Busca a leitura mais recente (últimas 24h) de cada uma das 12 estações
// mapeadas, já convertida e mapeada de volta pro nosso slug. Nunca lança:
// sem credencial configurada, ou qualquer falha de rede/autenticação,
// retorna [] — quem chama (coletar.js) não precisa de try/catch extra.
export async function buscarNiveisAna() {
  ...
  for (const resultado of resultados) {
    ...
    // Mais de um registro por estação na janela de 24h — fica só com o
    // mais recente por Data_Hora_Medicao, mesmo critério de "leitura
    // atual" usado no resto do sistema.
    const maisRecentePorCodigo = new Map();
    for (const item of resultado.value) {
      const anterior = maisRecentePorCodigo.get(item.codigoestacao);
      if (!anterior || item.Data_Hora_Medicao > anterior.Data_Hora_Medicao) {
        maisRecentePorCodigo.set(item.codigoestacao, item);
      }
    }

    for (const item of maisRecentePorCodigo.values()) {
      ...
      leituras.push({ slug, nivel, medidoEm });
    }
  }
