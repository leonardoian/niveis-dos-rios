...
// Exportadas (não só usadas internamente) pra dar pra testar isoladas em
// tests/feed.test.js, sem precisar simular o feed inteiro.
export function extrairSlug(url) {
  const caminho = new URL(url).pathname.replace(/\//g, '');
  return caminho === '' ? 'portoalegre' : caminho;
}

export function extrairNivel(texto) {
  // A casa decimal é opcional: quando o nível cai num número redondo, o feed
  // manda "1 metros" em vez de "1.00 metros" — sem o `?` essa leitura não
  // batia na regex e sumia da coleta sem erro nenhum.
